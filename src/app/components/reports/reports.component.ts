...
export class ReportsComponent {
  ...
  onFormSubmitted(value: ReportsFormValues) {
    ...
    this.briefingData$ = this.reportsService.getReports({
      ...
      map((response) => {
        ...
        const results = Object.entries(groupedData).map(([id, reports]) => ({id, reports}));
  ...
  private getReportParams = (value: ReportsFormValues) => {
    const mappedMandatoryValues = {
      reportTypes: mapFormDataMessageTypeToRequestDataReportType(value.messageTypes),
    };
